...
TournamentsWidget::TournamentsWidget(Client* client, QWidget* parent) : _client(client),_parent(parent){
...
void TournamentsWidget::pause() {}
void TournamentsWidget::resume() {}
...
void TournamentsWidget::quit() {
	_bidButton->setEnabled(false);
	_quitButton->setEnabled(false);
	_hasLeft=true;
}

void TournamentsWidget::endOfTurn() {
	++_currentTurn;
	_updater->stop();
	...
	if (result<0) {
		if (result==-1) {
			...
			connect(timer, SIGNAL(timeout()), this, SLOT(endAuction()));
			timer->start();
		}
	 	else if (result==-2) {
...
void TournamentsWidget::prepareForNextTurn(){
	--_timeBeforeTurn;
...
void TournamentsWidget::prepareForEndOfTurn(){
...
void TournamentsWidget::changeLabel(){
...
void TournamentsWidget::startTurn() {
...
void TournamentsWidget::endAuction(){
	_listAuctionsWidget->update();
	_listAuctionsWidget->show();
	_listAuctionsWidget->resume();
	
	init();
	dynamic_cast<OfficePage*>(_parent)->showStack();
}